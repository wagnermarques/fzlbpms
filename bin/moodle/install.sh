...
# Versão do Moodle a ser clonada do Git
...
# --- Configurações do Banco de Dados (para fzl-postgresql) ---
DB_TYPE="pgsql"
...
MOODLE_APP_PATH_IN_CONTAINER="/var/www/html/moodle" # Caminho DO Moodle dentro do contêiner Nginx/PHP-FPM
MOODLE_DATAROOT_IN_CONTAINER="/var/www/moodledata" # Caminho DO moodledata dentro do contêiner PHP-FPM

MOODLE_ADMIN_USER="admin"
MOODLE_ADMIN_USER_PASS="SuaSenhaAdminForte" # MUDE ESTA SENHA!
MOODLE_SITE_FULLNAME="EtecZL PPPs"
MOODLE_SITE_SHORTNAME="Escola Tecnica Estadual Zona Leste (Progressões Parciais)"
...
# Usuário do host que precisa ter acesso aos arquivos do Moodle
...
# Define códigos de cores para uma saída mais legível
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
NC='\033[0m' # Sem Cor

echo -e "${YELLOW}Iniciando a instalação e configuração do Moodle...${NC}"


# --- 1. Instalar dependências no host ---
# checka se o git está instalado caso não esteja, informa o usuario do script e para a execucao
if ! command -v git &> /dev/null; then
    echo -e "${RED}Erro: Git não está instalado. Por favor, instale o Git antes de continuar.${NC}"
    exit 1
fi
...
if [ -d "$MOODLE_APP_HOST_PATH" ]; then
    # Se o diretório já existe, removemos para garantir um clone limpo
    # Isso é importante para evitar conflitos de versão ou arquivos antigos
    # nao remove a pasta, simplesmente renomeia por seguranca
    echo -e "${YELLOW}O diretório '${MOODLE_APP_HOST_PATH}' já existe. Renomeando pasta antiga para um clone limpo...${NC}"
    sudo mv $MOODLE_APP_HOST_PATH "${MOODLE_APP_HOST_PATH}_backup_$(date +%Y%m%d_%H%M%S)"
fi

echo "Clonando Moodle (versão ${MOODLE_VERSION_TO_CLONE})... Isso pode levar um momento."
...
# --- 3. Definir permissões no host (nao no container) para os arquivos Moodle clonados no host ---
echo -e "\n${YELLOW}[5/6] Definindo permissões iniciais para os arquivos do Moodle no host...${NC}"
...
echo -e "${GREEN}Permissões de arquivos Moodle definidas.${NC}"


# --- 4. preparar banco de dados
# --- Verifica se o banco de dados já existe
echo -e "\n${YELLOW}[5/6] Verificando se o banco de dados '${DB_NAME}' já existe...${NC}"
if docker exec -it fzl-postgresql psql -U "${DB_USER}" -d postgres -c "\l" | grep -q "${DB_NAME}"; then
    echo -e "${YELLOW}O banco de dados '${DB_NAME}' já existe. Banco sera renomeado.${NC}"
    ...
    echo -e "${YELLOW}O banco de dados '${DB_NAME}' não existe. Criando o banco de dados...${NC}"
    docker exec -it fzl-postgresql psql -U "${DB_USER}" -d postgres -c "CREATE DATABASE ${DB_NAME} WITH OWNER ${DB_USER} ENCODING 'UTF8' TEMPLATE template0;"
...
# criação do dataroot e geração do config.php
...
if ! docker ps --filter "name=${PHP_FPM_CONTAINER_NAME}" --format "{{.ID}}" | grep -q .; then
    echo -e "${RED}Erro: Contêiner '${PHP_FPM_CONTAINER_NAME}' não está rodando. Por favor, inicie-o primeiro.${NC}"
    exit 1
fi
...
echo -e "\n${GREEN}=========================================${NC}"
echo -e "${GREEN}  Configuração do Moodle concluída!  ${NC}"
