...
public class ListGroupDevicesProcessor implements Processor {

	private static final String SELECT_SQL = "SELECT device_id, user_name FROM device_status "
		...
		try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_SQL)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				...
				while (rs.next()) {
					...
					json.append("\"userName\":").append(Json.nullableString(rs.getString("user_name")));
					json.append("}");
				}
			}
		}
